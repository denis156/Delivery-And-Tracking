...
<div>
    {{-- HEADER --}}
    <x-header title="Daftar Pengguna" subtitle="Halaman manajemen pengguna" icon="phosphor.users-four-duotone"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                @foreach($users as $user)
                    ...
                    >
                        ...
                        <x-slot:menu>
                            <div aria-label="{{ $user->status_color }}" class="status status-{{ $user->status_color }} animate-pulse"></div>
                ...
                <div class="mt-8 border-t border-base-300 pt-6">
                    ...
                    <div class="flex justify-center">
                        <div class="join shadow-sm">
                            ...
                            @php
                                $maxPages = 4;
                                $start = max($users->currentPage() - intval($maxPages/2), 1);
                                $end = min($start + $maxPages, $users->lastPage());
                                $start = max($end - $maxPages, 1);
                            @endphp

                            @if($start > 1)
                                <button wire:click="gotoPage(1)" class="join-item btn btn-sm hover:btn-primary hidden md:flex">1</button>
                                @if($start > 2)
                            ...
                            @for($i = $start; $i <= $end; $i++)
                                @if($i == $users->currentPage())
                                    <button class="join-item btn btn-sm btn-primary min-w-[2.5rem]">{{ $i }}</button>
                                @else
                                    <button wire:click="gotoPage({{ $i }})" class="join-item btn btn-sm hover:btn-primary min-w-[2.5rem]">{{ $i }}</button>
                                @endif
                            @endfor
                            ...
                            @if($end < $users->lastPage())
                                @if($end < $users->lastPage() - 1)
                                    <span class="join-item btn btn-sm btn-disabled cursor-default hidden md:flex">...</span>
                                @endif
                                <button wire:click="gotoPage({{ $users->lastPage() }})" class="join-item btn btn-sm hover:btn-primary hidden md:flex">{{ $users->lastPage() }}</button>
                            @endif
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        @if($statusFilter !== 'all')
                            <div class="flex items-center gap-2">
                                ...
                                <span>Status: <strong>{{ $statusFilter === 'active' ? 'Aktif' : 'Nonaktif' }}</strong></span>
                            </div>
                        @endif
                        @if($roleFilter !== 'all')
                            <div class="flex items-center gap-2">
                        ...
                        @if($sortBy['column'] !== 'created_at' || $sortBy['direction'] !== 'desc')
                            <div class="flex items-center gap-2">
                                ...
                                <span>Urutan:
                                    <strong>
                                        @php
                                            $sortLabel = collect($sortOptions)->where('id', $sortBy['column'])->first()['name'] ?? 'Unknown';
                                        @endphp
                                        {{ $sortLabel }}
