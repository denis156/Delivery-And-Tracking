    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        @foreach ($stats as $stat)
            <x-stat title="{{ $stat['title'] }}" value="{{ number_format($stat['value']) }}" icon="{{ $stat['icon'] }}"
                color="{{ $stat['color'] }}"
                class="{{ $stat['bg'] }} hover:shadow-xl hover:shadow-primary transition-all duration-300" />
        @endforeach
    </div>
            ...
            <x-card title="Surat Jalan Terbaru" shadow separator class="shadow-md h-full flex flex-col">
                ...
                <div class="space-y-4 flex-1 overflow-y-auto">
                    @forelse($recentDeliveries->take(6) as $delivery)
                        <div
                            class="flex items-center justify-between p-4 bg-base-300 rounded-lg hover:shadow-xl hover:shadow-primary transition-all duration-300">
                            <div class="flex-1">
            ...
            <x-card title="Truck Aktif" shadow separator class="h-full flex flex-col">
                ...
                <div class="space-y-4 flex-1 overflow-y-auto">
                    @foreach ($activeTrucks as $truck)
                        <div class="p-3 bg-base-300 rounded-lg hover:shadow-xl hover:shadow-primary transition-all duration-300">
